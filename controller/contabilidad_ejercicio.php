...
/*	  $partida0 = new partida();
	  ...
	  $part = $partida0->correccion_debe_negativos();
		...
		$num=0;
		$sub = $subcuenta->all($num);	
		print '<script language="JavaScript">'; 
					print 'alert(" cant '.count($sub).'  ");'; 
					print '</script>';
				foreach($sub as $su)
			{
								
							$par = $partida0->totales_from_subcuenta_all($su->idsubcuenta);
				if($par)
				{			
							$subc = $subcuenta1->get($su->idsubcuenta);
							$subc->debe = $par['debe'];
							$subc->haber = $par['haber'];
							$subc->modificar_sunas_debe_haber();
				}						
			}
*/	  
